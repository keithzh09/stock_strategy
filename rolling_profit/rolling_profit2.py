...
class RollingProfitStrategy:
    ...
    def run(self):
        ...
        for items in data.panel_gen:  # 每一天
            # 股票池中所有未入场的股票现金均分一下
            not_bought_stocks = []
            all_cash = self.Account.cash_available
            all_in_bought_cash = 0
            for stock_code in self.stock_pool:
                if not self.bought_stock[stock_code]:
                    not_bought_stocks.append(stock_code)
                else:
                    all_in_bought_cash += sum(self.init_cash[stock_code])   # 已经入场的股票的已经分配的金额不会改变
            # print(self.Account.cash_available)
            for stock_code in not_bought_stocks:
                # 所有现金减去已经入场的股票建仓的现金和
                self.init_cash[stock_code] = [(all_cash - all_in_bought_cash) / len(not_bought_stocks) / self.cash_n for
                                              i in range(self.cash_n)]
            for item in items.security_gen:
                ...
                if not self.bought_stock[stock_code]:  # 股票未入场的情况
                    if self.if_buy(stock_code, str(date)):  # 若是可以买
                        if self.init_cash[stock_code][0] != 0:
                            order = self.Account.send_order(
                                code=stock_code,
                                time=date,
                                money=self.init_cash[stock_code][0],
                            ...
                            self.init_cash[stock_code][0] = 0    # 简单设置为0
                    ...
                    if (close_price - self.bought_price[stock_code]) / close_price > 0.15 \
                        ...
                        if (close_price - self.last_price[stock_code]) / close_price >= 0.01:  # 日涨1%(今日比昨日涨1%，才算)
                            for i in range(self.cash_n):
                                if self.init_cash[stock_code][i] != 0:
                                    order = self.Account.send_order(
                                        code=stock_code,
                                        time=date,
                                        money=self.init_cash[stock_code][i],
                                        towards=QA.ORDER_DIRECTION.BUY,
                                        price=close_price,
                                        order_model=QA.ORDER_MODEL.CLOSE,
                                        amount_model=QA.AMOUNT_MODEL.BY_MONEY
